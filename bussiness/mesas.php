...
class clsMesa {
	...
	function Listar($tipo, $param1 = '', $param2 = '', $param3 = '', $param4 = ''){
		...
		elseif (($tipo === 'ATENCION') || ($tipo === 'TV') || ($tipo === 'COCINA') || ($tipo === 'NOTIFCOCINA') || ($tipo === 'NOTIFTV') || ($tipo === 'NOTIFATENCION')) {
			...
			$tabla .= 'LEFT JOIN ( ';
			$tabla .= 'SELECT a.tm_idmesa, a.tm_idatencion, a.FechaAct, b.ta_tipoubicacion ';
			$tabla .= 'FROM td_atencion as a ';
			...
			$campos = array(
				...
				'IFNULL(c.ta_tipoubicacion, \'00\') AS ta_tipoubicacion');
			...
			if ($tipo != 'ATENCION' ){
				...
				elseif (($tipo === 'TV') || ($tipo === 'NOTIFTV')){
					$condicion .= ' and NOT a.ta_estadoatencion IN (\'00\', \'07\')';
					if ($tipo == 'NOTIFTV')
						$limit = ' 0, 1 ';
				}
		...
		elseif (($tipo == 'UNIDAS') || ($tipo == 'UNIDAS-COCINA')){
			...
			$campos = array('a.tm_idmesa', 'a.tm_codigo', 'a.tm_nombre', 'a.tm_idambiente', 'b.tm_idatencion', 'a.ta_estadoatencion', 'd.ta_colorleyenda', 'c.ta_tipoubicacion');
	...
	function ListarGroupMesas($tipo, $idempresa, $idcentro, $idambiente, $estadoatencion)
	{
		$bd = $this->objData;
		$rs = $bd->exec_sp_select('pa_grupo_mesa_listar', array($tipo, $idempresa, $idcentro, $idambiente, $estadoatencion));
		return $rs;
	}

	/***************************************************/

	function ListarMesas($tipo, $idempresa, $idcentro, $id, $criterio, $pagina)
	{
		$bd = $this->objData;
		$rs = $bd->exec_sp_select('pa_mesa_listar', array($tipo, $idempresa, $idcentro, $id, $criterio, $pagina));
		return $rs;
	}

	function RegistrarMesas($idmesa, $idempresa, $idcentro, $idambiente, $codigo, $nombre, $tiene_correlativo, $nrocomensales, $idusuario, &$rpta, &$titulomsje, &$contenidomensaje)
	{
		$bd = $this->objData;
		$sp_name = 'pa_mesa_registrar';
		$result = $bd->exec_sp_iud($sp_name, array($idmesa, $idempresa, $idcentro, $idambiente, $codigo, $nombre, $tiene_correlativo, $nrocomensales, $idusuario), '@rpta, @titulomsje, @contenidomensaje');
	...
	function EliminarMesas($idmesa, $idusuario, &$rpta, &$titulomsje, &$contenidomensaje)
	{
		$bd = $this->objData;
		$sp_name = 'pa_mesa_eliminar';
		$result = $bd->exec_sp_iud($sp_name, array($idmesa, $idusuario), '@rpta, @titulomsje, @contenidomensaje');
		$rpta = $result[0]['@rpta'];
		$titulomsje = $result[0]['@titulomsje'];
		$contenidomensaje = $result[0]['@contenidomensaje'];
		return $rpta;
	...
	function ActualizarEstadoMesas($idempresa, $idcentro, $tipomesa, $estadomesa, $idmesas, $idusuario, &$rpta, &$titulomsje, &$contenidomensaje)
	{
		$bd = $this->objData;
		$sp_name = 'pa_mesa_estado_actualizar';
		$result = $bd->exec_sp_iud($sp_name, array($idempresa, $idcentro, $tipomesa, $estadomesa, $idmesas, $idusuario), '@rpta, @titulomsje, @contenidomensaje');
		$rpta = $result[0]['@rpta'];
		$titulomsje = $result[0]['@titulomsje'];
		$contenidomensaje = $result[0]['@contenidomensaje'];
		return $rpta;

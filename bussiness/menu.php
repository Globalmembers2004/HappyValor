...
class clsMenu {
	...
	function ListMenuPerfil($tipo, $idempresa, $idcentro, $idperfil, $idreferencia, $tipomenu)
	{
		$bd = $this->objData;
		$rs = $bd->exec_sp_select('pa_perfil_menu_listar', array($tipo, $idempresa, $idcentro, $idperfil, $idreferencia, $tipomenu));
		return $rs;
	}

	function ListarMenuPerfilGroup($tipo, $idperfil, $tipomenu, $idreferencia)
	{
		$bd = $this->objData;
		$rs = $bd->exec_sp_select('pa_perfilmenu_group_home', array($tipo, $idperfil, $tipomenu, $idreferencia));
		return $rs;
	}

...
class clsAmbiente {
	...
	function ListarAmbientes($tipo, $idempresa, $idcentro, $id, $criterio, $pagina)
	{
		$bd = $this->objData;
		$rs = $bd->exec_sp_select('pa_ambiente_listar', array($tipo, $idempresa, $idcentro, $id, $criterio, $pagina));
		return $rs;
	}

	function RegistrarAmbientes($idambiente, $idempresa, $idcentro, $codigo, $nombre, $idusuario, &$rpta, &$titulomsje, &$contenidomensaje)
	{
		$bd = $this->objData;
		$sp_name = 'pa_ambiente_registrar';
		$result = $bd->exec_sp_iud($sp_name, array($idambiente, $idempresa, $idcentro, $codigo, $nombre, $idusuario), '@rpta, @titulomsje, @contenidomensaje');
		$rpta = $result[0]['@rpta'];
	...
	function EliminarAmbientes($idambiente, $idusuario, &$rpta, &$titulomsje, &$contenidomensaje)
	{
		$bd = $this->objData;
		$sp_name = 'pa_ambiente_eliminar';
		$result = $bd->exec_sp_iud($sp_name, array($idambiente, $idusuario), '@rpta, @titulomsje, @contenidomensaje');
		$rpta = $result[0]['@rpta'];
		$titulomsje = $result[0]['@titulomsje'];
		$contenidomensaje = $result[0]['@contenidomensaje'];
		return $rpta;

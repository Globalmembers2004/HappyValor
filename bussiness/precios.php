...
class clsPrecio {
	...
	function Registrar__Articulo($idempresa, $idcentro, $idproducto, $precio, $idusuario, &$rpta, &$titulomsje, &$contenidomensaje)
	{
		$bd = $this->objData;
		$sp_name = 'pa_precio_articulo_registrar';
		$result = $bd->exec_sp_iud($sp_name, array($idempresa, $idcentro, $idproducto, $precio, $idusuario), '@rpta, @titulomsje, @contenidomsje');
		$rpta = $result[0]['@rpta'];

...
if ($tipobusqueda == 'M')
	$row = $objequipo->Listarequipos($tipo, $IdEmpresa, $IdCentro, $id, $criterio, $pagina);
elseif (($tipobusqueda == 'ATENCION') || ($tipobusqueda == 'NOTIFTV') || ($tipobusqueda == 'NOTIFATENCION')){
	if (($tipobusqueda == 'NOTIFTV') || ($tipobusqueda == 'NOTIFATENCION'))
		$row = $objequipo->Listar($tipobusqueda, $IdEmpresa, $IdCentro);
	else
		$row = $objAtencion->ListarPedidos($tipobusqueda, $IdEmpresa, $IdCentro, $idambiente, '');
		//$row = $objequipo->Listar($tipobusqueda, $IdEmpresa, $IdCentro, $idambiente);
}
elseif ($tipobusqueda == 'NOTIFCOCINA')
	$row = $objequipo->Listar($tipobusqueda, $IdEmpresa, $IdCentro, '\'03\'');
elseif ($tipobusqueda == 'UNIDAS')
	$row = $objequipo->Listar($tipobusqueda, $IdEmpresa, $IdCentro);
elseif ($tipobusqueda == 'GROUP-equipos')
	$row = $objequipo->ListarGroupequipos($tipo, $IdEmpresa, $IdCentro, $idambiente, $estadoatencion);

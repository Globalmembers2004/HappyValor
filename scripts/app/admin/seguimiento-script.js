                ...
                buttons: [
                    {
                        ...
                        content: 'Eliminar',
                        onClickButton: function (event) {
                            EliminarItemseguimiento(parent[0], 'single');
                        }
                    }
                ],
                cancelButton: true
...
function GoToEdit (idItem) {
    ...
    openModalCallBack(selectorModal, function () {
        ...
        if (idItem == '0') {
            ...
            $.ajax({
                type: "GET",
                url: 'services/seguimiento/seguimiento-getdetails.php',
                cache: false,
                dataType: 'json',
                data: 'id=' + idItem,
                success: function (data) {
                    if (data.length > 0){
                        ...
                        $('#hdIdPrimary').val(data[0].tm_idseguimiento);
                        ...
                        $('#txtIngresos').val(data[0].tm_ingresos);
                        $('#txtMeta').val(data[0].tm_meta);
                        $('#txtFaltante').val(data[0].tm_faltante);
                        $('#txtPorcentaje').val(data[0].tm_Porcentaje);
                        $('#txtObservacion').val(data[0].tm_observacion);
                        
                        Listarequilibrio_Combo(data[0].tm_idzonacorporal);

                        $('#ddlPeriodo').val(data[0].tm_idequilibrio).focus();
...
function BuscarDatos (pagina) {
    ...
    $.ajax({
        type: "GET",
        url: "services/seguimiento/seguimiento-search.php",
        cache: false,
        dataType: 'json',
        data: {
        ...
        success: function(data){
            ...
            if (countdata > 0){
                while(i < countdata){
                    ...
                    var idItem = data[i].tm_idseguimiento;
...
function GuardarDatos () {
    ...
    //if ($('#form1').valid()){
        ...
        $.ajax({
            type: "POST",
            url: 'services/seguimiento/seguimiento-post.php',
            contentType:false,
            processData:false,
            cache: false,
            data: data,
...
function Eliminarseguimiento () {
    ...
    EliminarItemseguimiento(elemsSelected[0], 'multiple');
}

function EliminarItemseguimiento (item, mode) {
    var data = new FormData();
    var idmodel = item.getAttribute('data-idmodel');

    data.append('btnEliminar', 'btnEliminar');
    data.append('hdIdseguimiento', idmodel);
            ...
            if (data.rpta == '0'){
                ...
                if (mode == 'multiple'){
                    ...
                    if (indexList <= elemsSelected.length - 1)
                        EliminarItemseguimiento(elemsSelected[indexList], mode);
                    else {
                        endqueue = true;
...
function Listarequilibrio_Combo (idequilibrio_default) {
    $.ajax({
        type: 'GET',
        ...
        success: function(data){
            ...
            if (countdata > 0) {
                while (i < countdata){
                    var _selected = idequilibrio_default == data[i].tm_idequilibrio ? ' selected' : '';
                    strhtml += '<option' + _selected + ' value="' + data[i].tm_idequilibrio + '">' + data[i].periodo + '</option>';
                    ++i;
                };
            };

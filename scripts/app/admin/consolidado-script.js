                ...
                buttons: [
                    {
                        ...
                        content: 'Eliminar',
                        onClickButton: function (event) {
                            EliminarItemConsolidado(parent[0], 'single');
                        }
                    }
                ],
                cancelButton: true
...
function LimpiarForm () {
    $('#txtDescripcion').val('PROCESO DE 2018');focus();
    $('#txtPeriodo').val('2018');
    $('#txtImporte').val('700');
    $('#txtSubdiario').val('');    
    $('#txtInicio').val('');
...
function removeValidFormRegister () {
    ...
    $('#txtImporte').rules('remove');
    $('#txtSubdiario').rules('remove');
...
function addValidFormRegister () {
    $('#txtDescripcion').rules('add', {
        ...
        minlength: 10,
        messages:{
            required: jQuery.validator.format("Por favor ingrese una descripción del periodo"),
    ...
    $('#txtImporte').rules('add', {
        required: true,
        minlength: 3,
        number: true 
    });

    $('#txtSubdiario').rules('add', {
        required: true,
        minlength: 2,
        maxlength: 2,        
        number: true 
...
function GoToEdit (idItem) {
    ...
    openModalCallBack(selectorModal, function () {
        ...
        if (idItem == '0')
            ...
            $.ajax({
                type: "GET",
                url: 'services/consolidado/consolidado-getdetails.php',
                cache: false,
                dataType: 'json',
                data: 'id=' + idItem,
                success: function (data) {
                    if (data.length > 0){
                        $('#hdIdPrimary').val(data[0].tm_idconsolidado);
                        $('#txtDescripcion').val(data[0].tm_descripcion);
                        $('#txtPeriodo').val(data[0].tm_periodo);
                        $('#txtImporte').val(data[0].tm_importe);
                        $('#txtSubdiario').val(data[0].tm_subdiario);
...
function BuscarDatos (pagina) {
    ...
    $.ajax({
        type: "GET",
        url: "services/consolidado/consolidado-search.php",
        cache: false,
        dataType: 'json',
        data: {
        ...
        success: function(data){
            ...
            if (countdata > 0){
                while(i < countdata){
                    var iditem = data[i].tm_idconsolidado;
...
function GuardarDatos () {
    ...
    if ($('#form1').valid()){
        ...
        $.ajax({
            type: "POST",
            url: 'services/consolidado/consolidado-post.php',
            contentType:false,
            processData:false,
            cache: false,
            data: data,
...
function EliminarConsolidado () {
    ...
    EliminarItemConsolidado(elemsSelected[0], 'multiple');
}

function EliminarItemConsolidado (item, mode) {
    var data = new FormData();
    var idmodel = item.getAttribute('data-idmodel');

    data.append('btnEliminar', 'btnEliminar');
    data.append('hdIdConsolidado', idmodel);
            ...
            if (data.rpta == '0'){
                ...
                if (mode == 'multiple'){
                    ...
                    if (indexList <= elemsSelected.length - 1)
                        EliminarItemConsolidado(elemsSelected[indexList], mode);
                    else {
                        endqueue = true;

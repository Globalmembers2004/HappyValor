                ...
                buttons: [
                    {
                        ...
                        content: 'Eliminar',
                        onClickButton: function (event) {
                            EliminarItemCambio(parent[0], 'single');
                        }
                    }
                ],
                cancelButton: true
...
function addValidFormRegister () {
    $('#txtFecha').rules('add', {
        ...
        maxlength: 10,
        messages:{
            required: jQuery.validator.format("Por favor ingrese la fecha"),
            minlength: jQuery.validator.format("Por favor ingrese la fecha en formato YYYY-MM-DD")
        }
    });

    $('#txtTipoCambio').rules('add', {
        required: true,
        number: true,
        minlength: 3,
        maxlength: 5,
        messages:{
            required: jQuery.validator.format("Por favor ingrese tipo de cambio"),
            minlength: jQuery.validator.format("Por favor ingrese tipo de cambio válido")
        }
    });
...
function GoToEdit (idItem) {
    ...
    openModalCallBack(selectorModal, function () {
        ...
        if (idItem == '0')
            ...
            $.ajax({
                type: "GET",
                url: 'services/cambio/cambio-getdetails.php',
                cache: false,
                dataType: 'json',
                data: 'id=' + idItem,
                success: function (data) {
                    if (data.length > 0){
                        $('#hdIdPrimary').val(data[0].tm_idcambio);
                        $('#txtFecha').val(data[0].tm_fecha);
                        $('#txtTipoCambio').val(data[0].tm_cambio);
...
function BuscarDatos (pagina) {
    ...
    $.ajax({
        type: "GET",
        url: "services/cambio/cambio-search.php",
        cache: false,
        dataType: 'json',
        data: {
        ...
        success: function(data){
            ...
            if (countdata > 0){
                while(i < countdata){
                    ...
                    strhtml += '<span class="title descripcion">' + data[i].tm_fecha + '</span>';
...
function GuardarDatos () {
    ...
    if ($('#form1').valid()){
        ...
        $.ajax({
            type: "POST",
            url: 'services/cambio/cambio-post.php',
            contentType:false,
            processData:false,
            cache: false,
            data: data,
...
function EliminarCambio () {
    ...
    EliminarItemCambio(elemsSelected[0], 'multiple');
}

function EliminarItemCambio (item, mode) {
    var data = new FormData();
    var idmodel = item.getAttribute('data-idmodel');

    data.append('btnEliminar', 'btnEliminar');
    data.append('hdIdCambio', idmodel);
            ...
            if (data.rpta == '0'){
                ...
                if (mode == 'multiple'){
                    ...
                    if (indexList <= elemsSelected.length - 1)
                        EliminarItemCambio(elemsSelected[indexList], mode);
                    else {
                        endqueue = true;

                ...
                buttons: [
                    {
                        ...
                        content: 'Eliminar',
                        onClickButton: function (event) {
                            EliminarItemInstructorgrupal(parent[0], 'single');
                        }
                    }
                ],
                cancelButton: true
...
function GoToEdit (idItem) {
    ...
    openModalCallBack(selectorModal, function () {
        ...
        if (idItem == '0')
            ...
            $.ajax({
                type: "GET",
                url: 'services/Instructorgrupal/Instructorgrupal-getdetails.php',
                cache: false,
                dataType: 'json',
                data: 'id=' + idItem,
                success: function (data) {
                    if (data.length > 0){
                        ...
                        $('#txtApellidos').val(data[0].tm_spellidos);
                        $('#txtDNI').val(data[0].tm_nrodni);
                        ...
                        $('#txtDNI').val(data[0].tm_nrodni).focus();
        ...
        success: function(data){
            ...
            if (countdata > 0){
                while(i < countdata){
                    ...
                    strhtml += '<span class="correo">Correo: ' + data[i].tm_email + '</span> - ' + data[i].tm_facebook + '</p>';
...
function EliminarInstructorgrupal () {
    indexList = 0;
    elemsSelected = $('#gvDatos .selected');
    EliminarItemInstructorgrupal(elemsSelected[0], 'multiple');
}

function EliminarItemInstructorgrupal (item, mode) {
    var data = new FormData();
    var idmodel = item.getAttribute('data-idmodel');

    data.append('btnEliminar', 'btnEliminar');
    data.append('hdIdInstructorgrupal', idmodel);
            ...
            if (data.rpta == '0'){
                ...
                if (mode == 'multiple'){
                    ...
                    if (indexList <= elemsSelected.length - 1)
                        EliminarItemInstructorgrupal(elemsSelected[indexList], mode);
                    else {
                        endqueue = true;

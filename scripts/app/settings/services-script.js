$(function () {
    $('#txtPrecio').on('focus', function(event) {
    ...
    $('#txtNombreServicio').on('keydown', function(event) {
...
function LimpiarForm () {
    ...
    $('#txtPrecio').val('0.00');
    $('#txtDias').val('0.00');
    $('#txtVencimiento').val('0.00');
    $('#chkMembresia').val(0);
    $('#txtNombreServicio').val('').focus();
}
...
function MostrarDatos () {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    strhtml += '<li class="mdl-list__item dato pos-rel" data-id="' + data[i].tm_idservicio + '">';
                    
                    strhtml += '<input name="chkItem[]" type="checkbox" class="oculto" value="' + data[i].tm_idservicio + '" />';
...
function GuardarDatos () {
    ...
    $.ajax({
        type: "POST",
        url: 'services/servicio/servicio-post.php',
        cache: false,
        contentType: false,
        processData: false,
        data: data,
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.length > 0) {
                        ...
                        $('#txtNombreServicio').val(data[0].tm_nombre);

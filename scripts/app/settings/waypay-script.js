...
function MostrarDatos () {
    $.ajax({
        url: 'services/formapago/formapago-search.php',
        type: 'GET',
        dataType: 'json',
        success: function (data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    strhtml += '<li class="mdl-list__item dato pos-rel" data-id="' + data[i].tm_idformapago + '">';
                    
                    strhtml += '<input name="chkItem[]" type="checkbox" class="oculto" value="' + data[i].tm_idformapago + '" />';
...
function GuardarDatos () {
    ...
    $.ajax({
        type: "POST",
        url: 'services/formapago/formapago-post.php',
        cache: false,
        contentType: false,
        processData: false,
        data: data,
...
function GetDataById (idData) {
    ...
    openModalCallBack('#modalRegistro', function () {
        if (idData != '0') {
            $.ajax({
                url: 'services/formapago/formapago-getdetails.php',
                type: 'GET',
                dataType: 'json',
                data: {
                    tipobusqueda: '2',
                ...
                success: function (data) {
                    if (data.length > 0) {
                        $('#hdIdPrimary').val(data[0].tm_idformapago);

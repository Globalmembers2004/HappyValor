$(function () {
    ...
    $('#txtAbreviatura').on('keydown', function(event) {
        if (event.keyCode == $.ui.keyCode.ENTER){
            return false;
        };
    });
        ...
        success: function (data) {
            ...
            if (countdata > 0){
                while(i < countdata){
                    ...
                    color_fila = '#f9fffd';
                    strhtml += '<li class="mdl-list__item dato pos-rel" data-id="' + data[i].tm_idtipofamilia + '"  data-baselement="' + selectorgrid + '" style="background-color: ' + color_fila + ';">';
                    
                    strhtml += '<input name="chkItem[]" type="checkbox" class="oculto" value="' + data[i].tm_idtipofamilia + '" />';
...
function GuardarDatos () {
    ...
    $.ajax({
        type: "POST",
        url: 'services/tipofamilia/tipofamilia-post.php',
        cache: false,
        contentType: false,
        processData: false,
        data: data,
    ...
    openModalCallBack('#modalRegistro', function () {
        if (idData != '0') {
            $.ajax({
                ...
                success: function (data) {
                    if (data.length > 0) {
                        $('#hdIdPrimary').val(data[0].tm_idtipofamilia);
                        $('#txtNombre').val(data[0].tm_descripcion);
                        $('#txtAbreviatura').val(data[0].tm_abreviacion);

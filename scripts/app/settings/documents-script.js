    ...
    $('#gvDatos').on('click', '.mdl-button', function(event) {
        ...
        if (accion == 'edit'){
            ...
            GetDataById(iditem);
             ...
             MessageBox({
                content: '¿Desea eliminar el tipo de documento de identificaci&oacute;n?',
                width: '320px',
                height: '130px',
                buttons: [
                    {
...
function MostrarDatos () {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    strhtml += '<li class="mdl-list__item dato pos-rel" data-id="' + data[i].tm_iddocident + '">';
                    
                    strhtml += '<input name="chkItem[]" type="checkbox" class="oculto" value="' + data[i].tm_iddocident + '" />';

                    strhtml += '<span class="mdl-list__item-primary-content">' + data[i].tm_descripcion + '</span>';
...
function GuardarDatos () {
    ...
    $.ajax({
        type: "POST",
        url: 'services/documentos/documentos-post.php',
        cache: false,
        contentType: false,
        processData: false,
        data: data,

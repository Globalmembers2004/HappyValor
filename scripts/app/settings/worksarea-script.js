$(function () {
    $('#gvDatos').on('click', '.mdl-button', function(event) {
        ...
        if (accion == 'edit'){
            ...
            MessageBox({
                content: '¿Desea eliminar el área de trabajo?',
                width: '320px',
                height: '130px',
                buttons: [
                    {
...
function MostrarDatos () {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (countdata > 0){
                while(i < countdata){
                    strhtml += '<li class="mdl-list__item dato pos-rel" data-id="' + data[i].tm_idarea + '">';
...
function GuardarDatos () {
    ...
    $.ajax({
        type: "POST",
        url: 'services/areas/areas-post.php',
        cache: false,
        contentType: false,
        processData: false,
        data: data,
    ...
    openModalCallBack('#modalRegistro', function () {
        if (idData != '0') {
            $.ajax({
                ...
                success: function (data) {
                    if (data.length > 0) {
                        $('#hdIdPrimary').val(data[0].tm_idarea);

    ...
    $('#gvDatos').on('click', '.mdl-button', function(event) {
        ...
        if (accion == 'edit'){
            ...
            GetDataById(iditem);
             ...
             MessageBox({
                content: '¿Desea eliminar la unidad de medida?',
                width: '320px',
                height: '130px',
                buttons: [
                    {
...
function addValidForm () {
    $('#txtNombre').rules('add', {
        ...
        maxlength: 150,
        remote: {
            url:  'services/unidadmedida/check-nombre.php',
            type: 'POST',
            data: {
                idempresa: function() {
...
function MostrarDatos () {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    strhtml += '<li class="mdl-list__item dato pos-rel" data-id="' + data[i].tm_idunidadmedida + '">';
                    
                    strhtml += '<input name="chkItem[]" type="checkbox" class="oculto" value="' + data[i].tm_idunidadmedida + '" />';
...
function GuardarDatos () {
    ...
    $.ajax({
        type: "POST",
        url: 'services/unidadmedida/unidadmedida-post.php',
        cache: false,
        contentType: false,
        processData: false,
        data: data,
        ...
        if (idData != '0') {
            $.ajax({
                ...
                data: {
                    tipo: 'L',
                    tipobusqueda: '2',
                    idunidadmedida: idData
                },
                success: function (data) {
                    if (data.length > 0) {
                        $('#hdIdPrimary').val(data[0].tm_idunidadmedida);

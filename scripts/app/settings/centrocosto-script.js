$(function () {
    ...
    $('#gvDatos').on('click', '.mdl-button', function(event) {
        ...
        if (accion == 'edit'){
            ...
            MessageBox({
                content: '¿Desea eliminar el centro de costo?',
                width: '320px',
                height: '130px',
                buttons: [
                    {
    ...
    $('#txtCentroCosto').on('keydown', function(event) {
        if (event.keyCode == $.ui.keyCode.ENTER){
            return false;
        };
    });
...
function MostrarDatos () {
    $.ajax({
        ...
        success: function (data) {
            ...
            if (countdata > 0){
                while(i < countdata){
                    strhtml += '<li class="mdl-list__item dato pos-rel" data-id="' + data[i].tm_idcencosto + '">';
...
function GuardarDatos () {
    ...
    $.ajax({
        type: "POST",
        url: 'services/centrocosto/centrocosto-post.php',
        cache: false,
        contentType: false,
        processData: false,
        data: data,

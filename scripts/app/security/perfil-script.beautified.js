...
function GetDataById(idData) {
    ...
    openModalCallBack("#modalRegistro", function() {
        if (idData == "0") ListarOpcionesMenu("0"); else {
            $.ajax({
                url: "services/perfil/perfil-search.php",
                type: "GET",
                dataType: "json",
        ...
        success: function(data) {
            ...
            if (data.rpta == "0") {
                ...
                $(item).fadeOut(400, function() {
                    $(this).remove();
                    if ($("#gvDatos .dato").length == 0) $("#gvDatos .gridview").html("<h2>No se encontraron registros</h2>");
                });
                if (mode == "multiple") {
                    ++indexList;
                    if (indexList <= elemsSelected.length - 1) EliminarItem(elemsSelected[indexList], mode); else {
                        endqueue = true;
...
function ListarOpcionesMenu(idperfil) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            } else $(selector).html('<tr><td colspan="2">No se encontraron resultados.</td></tr>');
...
function treeMenu(data, mom, level) {
    ...
    for (var k in data) {
        if (data[k].tm_idmenuref == mom) {
            ...
            strhtml += '<input id="hdIdMenu' + k + '" name="hdIdMenu[]"  class="filled-in" type="checkbox" value="' + data[k].tm_idmenu + '"' + checking + ' /><label for="hdIdMenu' + k + '"></label>';

$(function () {
    ...
    $('#btnBackCaja').on('click', function(event) {
        ...
        // $('.mdl-card[data-id="' + module_id + '"]', window.top.document)
    });
    ...
    $('#txtSearchProd').keyup(function(event) {
        ...
        AlternarBusquedaItems(); // $('#btnSearchProducts').trigger('click');
            // return false;
        // }
...
function mostrarAlmacen (flag) {
    if (flag)
        $('#gpAlmacen').removeClass('oculto');
    else
        $('#gpAlmacen').addClass('oculto');
}
...
function AlternarBusquedaItems () {
    if (!$('#rogers-panel').hasClass('is-active'))
        ListarInsumos('1');
    // else if ($('#lannisters-panel').hasClass('is-active'))
    //     ListarArticulos('1');
}
...
function CalcularTotal () {
    ...
    var total_baseimponible = total_conimpuesto;
    var total_impuesto = 0;
...
function Guardar () {
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if (data.rpta != '0'){
                // if ($('#chkIngresoAutomatico')[0].checked)
                BackToList();
                // else {
...
function AgregarItemsCompra () {
    ...
    createSnackbar('Agregado correctamente');
}
...
function ListarInsumos (pagina){
    ...
    if ($('#tabAddItemsCompra .mdl-tabs__tab:first-child').hasClass('is-active'))
        tipo = '1';
    else
        tipo = '2';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (countdata > 0) {
                ...
                if (pagina == '1')
                    $(selectorgrid).html(strhtml);
                else
                    $(selectorgrid).append(strhtml);

                $(selectorgrid + ' .grouped-buttons a.tooltipped').tooltip();
            }
            else {
                if (pagina == '1')
                    $(selectorgrid).html('<h2>No se encontraron resultados.</h2>');
            };

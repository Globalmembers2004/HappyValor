$(function() {
    ...
    if (screenmode == "cliente") {
        InitBuscarUbicacion();
        $("#btnActualizarCentro__OnEmpty, #btnActualizarCentro").on("click", function(event) {
            event.preventDefault();
            ListarCentros();
        });
        $("#gvCentros").on("click", 'button[data-action="take-table"]', function(event) {
            event.preventDefault();
            var elem = getParentsUntil(this, "#gvCentros", ".dato");
            elem = elem[0];
            var _idcentro = elem.getAttribute("data-id");
            Ambientes(_idcentro);
        });
    ...
    //        };
    // });
    $("#btnBackToRooms").on("click", function(event) {
        event.preventDefault();
        BackToRooms();
    });
    ...
    $("*#gvArticuloMenu tbody").on("click focus", "tr .mdl-textfield__input", function(event) {
        if (event.type == "click") event.stopPropagation(); else this.select();
    });
...
var handlerRowCheck = function(_this, selectorgrid, event) {
    if ((event.target || {}).type !== "textbox") {
        ...
        if (selectorgrid == "#gvArticuloMenu") selector_checkbox = ".check-articulo";
        ...
        if (_parent != null) {
            if (selectorgrid == "#gvArticuloMenu") {
                ...
                if (_checkbox.checked) {
                    ...
                    if ($(selectorgrid).find(".selected").length == 0) $("#btnAddArticles").addClass("hide");
                }
            }
        }
    }
...
function MostrarAmbientes(idcentro) {
    var _idcentro = typeof idcentro !== "undefined" ? idcentro : $("#hdIdCentro").val();
    $.ajax({
        ...
        success: function(data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    ...
                    // start body slide
                    strhtml += '<div class="mdl-grid mdl-grid--no-spacing all-height">';
                    // start grid content slide
                    strhtml += '<div class="mdl-cell mdl-cell--6-col all-height">';
                    // start 1st column slide
                    strhtml += '<div class="mesas scrollbarra" class="gridview all-height" data-selected="none" data-multiselect="false" data-actionbar="mesas-actionbar"><div class="gridview-container mdl-grid"></div></div>';
                    strhtml += "</div>";
                    // end 1st column slide
                    strhtml += '<div class="mdl-cell mdl-cell--6-col">';
                    // start 2st column slide
                    strhtml += '<div class="grupos scrollbarra" class="gridview all-height" data-selected="none" data-multiselect="false" data-actionbar="grupos-actionbar"><div class="gridview-container mdl-grid"></div></div>';
                    strhtml += "</div>";
                    // end 2st column slide
                    strhtml += "</div>";
                    // end grid content slide
                    strhtml += "</div>";
                    // end body slide
                    strhtml += "</div>";
                    strhtml += "</div>";
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    ...
                    if (secciones.length == 1) {
                        if (secciones[0].td_nombreseccion.trim().length == 0) count_secciones = 0; else count_secciones = 1;
...
function showMenuRestaurante() {
    ...
    if (tipomenudia == "view-menu") ListarArticulos_Menu("1"); else if (tipomenudia == "view-card") ListarArticulos_Carta("1"); else ListarArticulos_Individual("1");
}

function Ambientes(idcentro) {
    $("#pnlLocal").fadeOut(300, function() {
        $("#pnlMesas").fadeIn(300, function() {
            // $('#pnlOrden > .gp-header .mdl-layout-title').text('Mesa: ' + nombremesa);
            MostrarAmbientes(idcentro);
        });
    });
}
...
function ListarMesas(idambiente, preload_overlayer) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    strhtml += '<div class="dato mdl-cell mdl-cell--3-col mdl-cell--2-col-phone pos-rel mdl-card" ';
        ...
        success: function(result) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    ...
                    strhtml += '<div class="dato mdl-cell mdl-cell--6-col mdl-cell--6-col-phone pos-rel mdl-card" ';
                    strhtml += 'data-idatencion="' + data[i].tm_idatencion + '" ';
                    strhtml += 'data-nroatencion="' + data[i].tm_nroatencion + '" ';
                    strhtml += 'data-idmesa="' + data[i].idgrupomesa + '" ';
                    strhtml += 'data-tipomesa="' + data[i].ta_tipomesa + '" ';
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (action == "view-menu") {
                ...
                if (action == "view-card") ListarArticulos_Carta("1"); else ListarArticulos_Individual("1");
            }
        },
        error: function(data) {
            console.log(error);
...
function initGvMesas() {
    gvMesas = new DataList("#carousel-example-generic .mesas", {
        actionbar: "#mesas-actionbar",
        onSearch: function() {},
        oneItemClick: function(event) {
            var elem = event.target;
            var item = getParentsUntil(elem, "#carousel-example-generic .mesas", ".dato");
            item = item[0];
            var idatencion = item.getAttribute("data-idatencion");
            var listmesa = item.getAttribute("data-idmesa");
            var tipomesa = item.getAttribute("data-tipomesa");
            var estadoatencion = item.getAttribute("data-state");
...
function InitBuscarUbicacion() {
    var registrandoPosicion = false, idRegistroPosicion, ultimaPosicionUsuario;
    //, marcadorUsuario, mapa, div = document.getElementById('mapa');
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (count_result > 0) {
                    ...
                    i = 0;
                    while (i < countdata) {
                        strhtml += '<div data-id="' + data[i].tm_idcentro + '" class="list-group-item dato">';
                        ...
                        if (i == 0) strhtml += '<button class="btn btn-primary waves-effect place-top-right tooltipped margin5" type="button" data-action="take-table" data-delay="50" data-position="bottom" data-tooltip="Ocupar mesa">Ocupar mesa</button>';
                        strhtml += "</div>";
                        ++i;
                    }
                    ...
                    $("#gvCentros .gridview-content").addClass("hide");
                    $("#gvCentros .empty_state").removeClass("hide");
                }
            },
            error: function(error) {
                console.log(error);
